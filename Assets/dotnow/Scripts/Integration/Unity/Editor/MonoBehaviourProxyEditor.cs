...
namespace dotnow
{
    ...
    {
        private bool _showSerializedValues;
        private int _depth;
        ...
        {
            _showSerializedValues = EditorGUILayout.Toggle("Show Serialized Values", _showSerializedValues);
            
            if (_showSerializedValues)
                base.OnInspectorGUI();

            var instance = ((ICLRProxy)target).Instance;
            object obj = instance;
            ...
            EditorGUILayout.LabelField($"{instance.Type.Name} (Interpreted Script)", GUI.skin.FindStyle("IN Title"));
            EditorGUI.indentLevel += 2;
            ...
            try
            {
                    ...
                    if (_foldouts[_path] = EditorGUILayout.Foldout(!_foldouts.ContainsKey(_path) || _foldouts[_path], name))
                    {
                        using (new EditorGUI.IndentLevelScope())
                        {
                            if (typeof(IList).IsAssignableFrom(type))
                            {
                                ...
                                {
                                    var o = list[i];
                                    if (DrawFieldRecursively(o, $"Element {i}", listType, ref o, null))
                                        list[i] = o;
                                }
                ...
                else
                {
                    ...
                    if (GUILayout.Button("Initialize field"))
                    {
                        try
                        {
                            value = type.IsCLRType() ? ScriptDomain.Active.GetAppDomain().CreateInstance((CLRType)type) : Activator.CreateInstance(type);
                        }
                        catch
                        {
